	...
	*buffers = atoi(argv[2]);
	...
	if(strcasecmp(argv[3], "STACK") == 0) {
		...
		*alg =BFF;
		...
		fprintf(stderr, "Scheduling algorithm must be one of the following options: STACK, BFF.\n");
		exit(1);
	}
}
...
void *consumer(void *arg) {
    ...
    while(1) {
        ...
        close(req->fd);// only consumers know when the client is servered.
        free(req);
    }
