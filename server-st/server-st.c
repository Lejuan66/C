...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        request *ptr_req = malloc(sizeof(request));
        //ptr_req->size = requestFileSize(connfd);
        /* TODO: Save the time for the statistics */
        ptr_req->arrival = gettimeofday(&arrival,NULL);
        ...
        requestHandle(ptr_req->fd,ptr_req->arrival, ptr_req->dispatch);
        /* TODO: Close */
        close(ptr_req->fd);
        free(ptr_req);
    }
