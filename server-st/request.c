...
void requestHandle(int fd, long arrival, long dispatch) {
    ...
    if (strcasecmp(method, "GET")) {
        requestError(fd, method, "501", "Not Implemented","GET method is not implemented");
        return;
    }
	...
	if (is_static) {
        ...
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
            requestError(fd, filename, "403", "Forbidden", "1DT032 Server could not  run this program??????");
            return;
	}
