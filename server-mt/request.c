...
int requestParseURI(char *uri, char *filename, char *cgiargs) {
	...
	if (!strstr(uri, "cgi")) {// strstr looking for the first subchar, strstr(*char chr, *char subchar). Common Gateway Interface/CGI使客户端可以通过浏览器向网络服务器请求数据
		/* Static */
        strcpy(cgiargs, "");//char *strcpy(char *dest, const char *src);strcpy()会将参数src 字符串拷贝至参数dest 所指的地址。如果参数 dest 所指的内存空间不够大，可能会造成缓冲溢出(buffer Overflow)的错误情况，在编写程序时请特别留意，或者用strncpy()来取代。
		sprintf(filename, ".%s", uri);
		if (uri[strlen(uri)-1] == '/') {
			strcat(filename, "home.html");// 连接两个字符串 strcat
		}
		return 1;
	} else {
		/* Dynamic */
